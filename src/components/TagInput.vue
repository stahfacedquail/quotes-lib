...
export default defineComponent({
    ...
    data() {
        ...
        updateTagSearchList() {
            this.tagSearchList = this.data.filter(value => value.name.toLowerCase().includes(this.tagValue.toLowerCase()));
        },
        updateTagList(result) {
            let res = {};
            
            if(typeof result == "string") {
                if(this.tagSearchList.length > 0 && result.toLowerCase() == this.tagSearchList[0].name) //matches an existing db entry
                    res = this.tagSearchList[0];
                else
                    //doesn't exist on db yet, so flag that db needs to create it
                    res = { "id": -1, "name": result };
            } else { //one of the tags that already as a db-assigned id
                //so check if it's a duplicate in the list of tags
                res = result;
            }

            if(this.tagListLowerCase.includes(res.name.toLowerCase())) {
                this.tagValue = "";
                return;
            }
